...
def change_balance(user_id, amount, kind, description, reference=None):
    ...
    with connection() as db:
        ...
        before, after = row['balance'], row['balance'] + int(amount)
        if after < 0:
            db.execute('ROLLBACK')
...
def buy_tickets(user_id, quantity):
    ...
    with connection() as db:
        ...
        if not account or account['is_banned'] or account['balance'] < total:
            db.execute('ROLLBACK')
            return {'ok': False, 'message': 'الرصيد غير كافٍ'}
        before, after = account['balance'], account['balance'] - total
...
def redeem(user_id, raw):
    ...
    with connection() as db:
        ...
        if not account or account['is_banned']:
            db.execute('ROLLBACK')
            return {'ok': False, 'message': 'الحساب غير متاح'}
        before, after = account['balance'], account['balance'] + gift['points']
        db.execute('UPDATE users SET balance=? WHERE user_id=?', (after, user_id))
...
def create_deposit(user_id, amount, method, proof):
    ...
    if amount <= 0 or not method.strip() or not proof.strip():
...
def create_withdrawal(user_id, amount, method, account):
    ...
    if amount < minimum or not method.strip() or not account.strip():
    ...
    with connection() as db:
        ...
        try:
            cur = db.execute('INSERT INTO withdrawal_requests(user_id,amount,method,account) VALUES(?,?,?,?)', (user_id, amount, method.strip(), account.strip()))
        except Exception:
            db.execute('ROLLBACK')
...
def spin_wheel(user_id):
    ...
    with connection() as db:
        ...
        prizes = [int(x.strip()) for x in setting('wheel_prizes', '25,50,100,250,500,1000').split(',') if x.strip()]
        if not prizes:
            db.execute('ROLLBACK')
