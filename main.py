...
def stats(message):
    ...
    with connection() as db:
        users=db.execute('SELECT COUNT(*) c FROM users').fetchone()['c']; balance=db.execute('SELECT COALESCE(SUM(balance),0) b FROM users').fetchone()['b']; pending_d=db.execute("SELECT COUNT(*) c FROM deposit_requests WHERE status='pending'").fetchone()['c']; pending_w=db.execute("SELECT COUNT(*) c FROM withdrawal_requests WHERE status='pending'").fetchone()['c']
    bot.send_message(message.chat.id,f'📊 المستخدمون: {users}\n💰 مجموع الأرصدة: {balance}\n📥 شحن معلق: {pending_d}\n📤 سحب معلق: {pending_w}')
