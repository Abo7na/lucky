...
def home(admin=False):
 ...
 for row in [[('💰 المحفظة','wallet'),('🎟️ اليانصيب','lottery')],[('🎮 الألعاب','games'),('🎡 العجلة','wheel')],[('🎁 كود هدية','gift'),('👤 حسابي','profile')],[('👥 الإحالة','referral'),('ℹ️ المساعدة','help')]]:m.row(*(button(*x) for x in row))
...
def lottery():
 m=types.InlineKeyboardMarkup(row_width=2);m.row(button('🎟️ تذكرة','ticket:1'),button('🎟️ خمس تذاكر','ticket:5'));m.add(button('✏️ كمية مخصصة','ticket_custom'));m.add(button('⬅️ رجوع','home'),button('🏠 الرئيسية','home'));return m
def wheel():return nav('home')
