...
def register(bot):
    ...
    def decide(message):
        ...
        if len(parts) != 2 or not parts[1].isdigit():
            bot.send_message(message.chat.id, "الصيغة: /approve_deposit ID أو /reject_withdraw ID")
            return
        ...
        if result.get("ok"):
            status = "تمت الموافقة" if approve else "تم الرفض"
            try:
                bot.send_message(result["user_id"], f"📌 {status} على طلبك رقم #{request_id}.\nالمبلغ: {result['amount']} نقطة")
            except Exception:
                pass
    ...
    def draw(message):
        ...
        if not result["ok"]:
            ...
            return
        bot.send_message(message.chat.id, f"✅ أُغلقت الجولة #{result['round_id']} وتم اختيار {len(result['winners'])} فائزين.")
        for winner in result["winners"]:
            try:
                bot.send_message(winner["user_id"], f"🎉 ربحت المركز {winner['place']} بمقدار {winner['prize']} نقطة!")
            except Exception:
                pass
    ...
    def ledger(message):
        ...
        with connection() as db:
            rows = db.execute("SELECT user_id,type,amount,reference,created_at FROM ledger ORDER BY id DESC LIMIT 30").fetchall()
