...
from services.wallet import change_balance, buy_tickets, redeem
...
def register(bot, admins):
    ...
    def start(message):
        u=user(message.from_user.id,message.from_user.username,message.from_user.first_name);bot.send_message(message.chat.id,'🎰 <b>مرحباً بك في Lucky</b>\nاختر من القائمة:',parse_mode='HTML',reply_markup=home(message.from_user.id in admins))
    @bot.message_handler(commands=['cancel'])
    def cancel(message): pending.pop(message.from_user.id,None);bot.clear_step_handler_by_chat_id(message.chat.id);bot.send_message(message.chat.id,'✖️ تم الإلغاء.',reply_markup=home(message.from_user.id in admins))
    @bot.callback_query_handler(func=lambda c: True)
    def callbacks(call):
        uid=call.from_user.id; data=call.data; bot.answer_callback_query(call.id)
        if data in ('home','cancel'): pending.pop(uid,None);bot.clear_step_handler_by_chat_id(call.message.chat.id);bot.edit_message_text('🏠 <b>القائمة الرئيسية</b>',call.message.chat.id,call.message.message_id,parse_mode='HTML',reply_markup=home(uid in admins));return
        if data=='wallet':
        ...
        if data in ('dice','coin'):
            cost=int(setting('game_cost','50'));reward=int(setting('game_reward','120'));win=random.randint(1,100)<=int(setting('game_chance','40'));r=change_balance(uid,reward if win else -cost,'GAME',data)
            if not r['ok'] and not win:r=change_balance(uid,-cost,'GAME',data)
            text=('🎉 ربحت '+str(reward)+' نقطة' if win else '❌ خسرت '+str(cost)+' نقطة') if r.get('ok') else '❌ '+r.get('message','فشلت العملية')
            bot.edit_message_text(text,call.message.chat.id,call.message.message_id,reply_markup=games());return
    ...
    def gift_input(message):
        ...
        r=redeem(message.from_user.id,message.text);bot.send_message(message.chat.id,'✅ أضيفت '+str(r.get('points'))+' نقطة' if r['ok'] else '❌ '+r['message'],reply_markup=home(message.from_user.id in admins))
