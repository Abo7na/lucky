import random
from database.db import connection, setting, user
from keyboards.common import games, home, lottery, nav, wallet, wheel
from services.wallet import buy_tickets, change_balance, create_deposit, create_withdrawal, redeem, spin_wheel
...
def register(bot, admins):
    ...
    def start(message):
        ...
        if ref and current['referred_by'] is None:
            ...
            if ref_exists and claimed.rowcount == 1:
                change_balance(ref, int(setting('referral_reward', '200')), 'REFERRAL', 'مكافأة إحالة', f'referral:{ref}:{uid}')
    ...
    def callbacks(call):
        ...
        if data in ('home', 'cancel'):
            ...
            bot.edit_message_text('🏠 <b>القائمة الرئيسية</b>', call.message.chat.id, call.message.message_id, parse_mode='HTML', reply_markup=home(uid in admins)); return
        if data == 'wallet':
        ...
        if data == 'wheel':
            bot.edit_message_text('🎡 يمكنك تدوير العجلة مرة كل 24 ساعة.', call.message.chat.id, call.message.message_id, reply_markup=wheel()); return
        if data == 'spin':
            result = spin_wheel(uid); text = f'🎉 ربحت {result["prize"]} نقطة' if result['ok'] else f'❌ {result["message"]}'; bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=wheel()); return
        if data == 'referral':
        ...
        if data == 'withdraw':
            msg = bot.send_message(uid, '➖ أرسل: المبلغ | الطريقة | الحساب', reply_markup=nav('wallet')); bot.register_next_step_handler(msg, withdraw_input); return
        if data == 'games':
        ...
        if data in ('dice', 'coin'):
            cost, reward = int(setting('game_cost', '50')), int(setting('game_reward', '120'))
            won = random.randint(1, 100) <= int(setting('game_chance', '40'))
            result = change_balance(uid, reward - cost if won else -cost, 'GAME', data)
            text = (f'🎉 ربحت {reward} نقطة' if won else f'❌ خسرت {cost} نقطة') if result.get('ok') else f'❌ {result.get("message", "فشلت العملية")}'; bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=games()); return
        if data == 'lottery':
            bot.edit_message_text('🎟️ اختر عدد التذاكر:', call.message.chat.id, call.message.message_id, reply_markup=lottery()); return
        if data.startswith('ticket:'):
            result = buy_tickets(uid, int(data.split(':')[1])); bot.edit_message_text('✅ تمت العملية' if result['ok'] else f'❌ {result["message"]}', call.message.chat.id, call.message.message_id, reply_markup=lottery()); return
        if data == 'ticket_custom':
            msg = bot.send_message(uid, 'أرسل عدد التذاكر أو /cancel'); bot.register_next_step_handler(msg, custom_ticket); return
        if data == 'gift':
    ...
    def custom_ticket(message):
        ...
        except (TypeError, ValueError): bot.send_message(message.chat.id, '❌ أرسل رقماً صحيحاً')

    def gift_input(message):
        if message.text == '/cancel': return cancel(message)
        result = redeem(message.from_user.id, message.text or '')
        bot.send_message(message.chat.id, f'✅ أضيفت {result.get("points", 0)} نقطة' if result['ok'] else f'❌ {result["message"]}', reply_markup=home(message.from_user.id in admins))

    def deposit_input(message):
        if message.text == '/cancel': return cancel(message)
        try: amount, method, proof = [x.strip() for x in (message.text or '').split('|', 2)]; result = create_deposit(message.from_user.id, int(amount), method, proof)
        except (ValueError, TypeError): result = {'ok': False, 'message': 'الصيغة: المبلغ | الطريقة | الإثبات'}
        if result['ok']:
            for admin in admins:
                try: bot.send_message(admin, f'📥 طلب شحن جديد #{result["id"]} من <code>{message.from_user.id}</code>\n/approve_deposit {result["id"]}', parse_mode='HTML')
    ...
    def withdraw_input(message):
        if message.text == '/cancel': return cancel(message)
        try: amount, method, account = [x.strip() for x in (message.text or '').split('|', 2)]; result = create_withdrawal(message.from_user.id, int(amount), method, account)
        except (ValueError, TypeError): result = {'ok': False, 'message': 'الصيغة: المبلغ | الطريقة | الحساب'}
        if result['ok']:
            for admin in admins:
                try: bot.send_message(admin, f'📤 طلب سحب جديد #{result["id"]} من <code>{message.from_user.id}</code>\n/approve_withdraw {result["id"]}', parse_mode='HTML')
                except Exception: pass
